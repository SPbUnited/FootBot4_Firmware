...
class EEPROMClass
{
  public:
    ...
    const T& put(const int& idx, const T& t)
    {
        ...
        {
            // Delay saving the data from the buffer to the flash. Just flag and wait for commit()
            // later
            _dirtyBuffer = true;
        }
     ...
     * Use this with care: Each and every commit will harm the flash and reduce it's lifetime (like
     * with every flash memory)
     */
    void commit()
    {
        if (!_initialized)
